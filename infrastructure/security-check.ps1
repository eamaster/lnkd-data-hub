...
Write-Host "Step 1: Checking for committed sensitive files..." -ForegroundColor Yellow
...
if ($sensitiveFiles) {
    ...
    Write-Host "PASS: No sensitive files in git" -ForegroundColor Green
}

# Check for RapidAPI key patterns
Write-Host ""
Write-Host "Step 2: Checking for hardcoded API keys..." -ForegroundColor Yellow
...
if ($apiKeyPattern) {
    ...
    Write-Host "PASS: No hardcoded API keys found" -ForegroundColor Green
}
...
Write-Host ""
Write-Host "Step 4: Checking local environment setup..." -ForegroundColor Yellow
if (Test-Path workers/.dev.vars) {
    Write-Host "PASS: workers/.dev.vars exists (for local dev)" -ForegroundColor Green
} else {
    Write-Host "WARNING: workers/.dev.vars missing - copy from .dev.vars.example" -ForegroundColor Yellow
}

if (Test-Path app/.env) {
    Write-Host "PASS: app/.env exists (for local dev)" -ForegroundColor Green
} else {
    Write-Host "WARNING: app/.env missing - copy from .env.example" -ForegroundColor Yellow
}
